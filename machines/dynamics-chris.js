...
var RewriteBase = "http://www.ihmmanniversary.org/"; 
...
var filesToVariablesArray = [
    ...
    {'output_person': 'views/output_person.php'},
        ...
        if((Object.keys(pagesCollection['pageData']).length) == pagesCollection.size){
            ...
            _.each(pageOrder, function(value, index){
                switch(index){
                    ...
                    returnJsonData('listSponsors', jsonArgs1).done(function(data){
                            if (!_.isEmpty(data)) {
                                _.each(data, function(value, index){
                                    returnSponsor = $(php_output_sponsor);
                                    $("#sponsors").find(".container").append(returnSponsor);
                                    if (!_.isNull(value.attachments)) {
                                    returnSponsor.find('.logo').find('img').attr('src', value.attachments[0].thumb);
                                    }
                                    returnSponsor.find(".sponsors_name").html(value.sponsors_name);
                                    returnSponsor.find(".sponsors_website").find('a').html(value.sponsors_website);
                                    returnSponsor.find(".sponsors_website").find('a').attr('href', "http://" + value.sponsors_website);
                                })
                            }
                    });
                    ...
                    case "people":
                        ...
                        returnJsonData('listPeople', jsonArgs1).done(function(data){
                            ...
                            if (!_.isEmpty(data)) {
                                _.each(data, function(value, index){
                                    returnPerson = $(php_output_person);
                                    $("#people").find(".container").append(returnPerson);
                                    if (!_.isNull(value.attachments)) {
                                    returnPerson.find('.photo').find('img').attr('src', value.attachments[0].thumb);
                                    }
                                    returnPerson.find(".first_name").html(value.first_name);
                                    returnPerson.find(".last_name").html(value.last_name);
                                    returnPerson.find(".work_title").html(value.work_title);
                                    returnPerson.find(".association_title").html(value.association_title);
                                    returnPerson.find(".company").html(value.company);
                                ...
                                //             case 'attachments':
                                //                 if(_.isNull(val)){
                                //                     returnPerson.find('.photo').remove();
                                //                 } else {
                                //                     _.each(val, function(v, k) {
                                //                         returnPerson.find('.photo').find('img').attr('alt', value.first_name + ' ' + value.last_name);
                                //                         returnPerson.find('.photo').find('img').attr('src', v.thumb);
                                //                     });
                                //                 }
                                //             break;

                                //             default:
                                //                 returnPerson.find('.' + k).html(val);
                                //             break;
                                //         }
                                //     });

                                //     returnObject.find('.all-people').append(returnPerson);
                                // });

                            }

                            // // Append returnObject
                            // $('#people').find('.container').append(returnObject)
                        });
                          ...
                          console.log(data);
                            if (!_.isEmpty(data)) {
                                _.each(data, function(value, index){
                                    ...
                                    $("#events").find(".container").append(returnEvent);

                                    // Date
                                    startDate = moment.unix(value.events_start_date).format("MMMM DD, YYYY h:mm A");
                                    endDate = moment.unix(value.events_end_date).format("h:mm A");

                                    returnEvent.find(".events_name").html(value.events_name);
                                    returnEvent.find(".events_location").html(value.events_location);
                                    returnEvent.find('.events_start_date').html(startDate);
                                    returnEvent.find('.events_end_date').html(endDate);
        
                                    returnEvent.find('.the_content').append(_.unescape(value.the_content));
            ...
            $(window).stellar();
            if(urlArray[1] != ""){
                if(!(urlArray[1] == defaultPage) && (window.pageYOffset == 0)){
                    goToByScroll(urlArray[1]);
                }
            }
        }
    }
